...
GameScene::GameScene(void) : m_grid("lvl/testLvl.dat", CELL_WIDTH, CELL_HEIGHT) {
	...
	candy = { { -100, -100, 100, 100 }, ObjectID::CANDY_BLUE };
	Pokemon = { { 0, 0, 100, 100 }, ObjectID::Pokemon };
}
...
void GameScene::Update(void) {
	...
	if (IM.IsKeyHold<KEY_BUTTON_UP>()) {
		Pokemon.transform.y -= TM.GetDeltaTime();
	}
	if (IM.IsKeyHold<KEY_BUTTON_DOWN>()) {
		Pokemon.transform.y += TM.GetDeltaTime();
	}
	if (IM.IsKeyHold<KEY_BUTTON_RIGHT>()) {
		Pokemon.transform.x += TM.GetDeltaTime();
	}
	...
	if (IM.IsKeyDown<KEY_BUTTON_SPACE>()){
		printf("space");
		candy.transform.x = Pokemon.transform.x;
		candy.transform.y = Pokemon.transform.y;
	}
	candy.transform.x += TM.GetDeltaTime();
}
